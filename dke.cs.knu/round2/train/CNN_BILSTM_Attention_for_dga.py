...
with tf.device("/GPU:0"):
    ...
    x_trains.to_csv(DATA_HOME + "split_2nd_train.csv", mode='w', index=False)
    x_tests.to_csv(DATA_HOME + "split_2nd_test.csv",mode='w', index=False)
...
with tf.device("/GPU:0"):
    ...
    def bidirection_lstm_with_attention(max_len=74, emb_dim=32, max_vocab_len=100, lstm_output_size=32, W_reg=regularizers.l2(1e-4)):
        ...
        output = Dense(2, activation='softmax', name='output')(hidden3)
...
with tf.device("/GPU:0"):
    ...
    model_name = "CNN_BILSTM_ATT_for_DGA"
    ...
    #print("Make dgalist")
    # Save test result
    df_Test = pd.read_csv(DATA_HOME + "split_2nd_test.csv",encoding='ISO-8859-1', sep=',')
    # df_Test = pd.read_csv("../../../data/split_2nd_test_sample.csv",encoding='ISO-8859-1', sep=',')
    #
