...
with tf.device("/GPU:0"):
    ...
    x_trains.to_csv("./trainsXBILSTM.csv", mode='w', index=False)
    x_tests.to_csv("./testsXBILSTM.csv",mode='w', index=False)
    # Sampling
...
with tf.device("/GPU:0"):
    ...
    def simple_lstm(max_len=74, emb_dim=32, max_vocab_len=100, lstm_output_size=32, W_reg=regularizers.l2(1e-4)):
        ...
        # LSTM layer
        lstm = Bidirectional(LSTM(lstm_output_size))(emb)
...
with tf.device("/GPU:0"):
    ...
    df_Test.to_csv("./result/BILSTM_result.csv",mode='w')
    #print("###############################WITH PREDICT")
    #print(df_Test)
